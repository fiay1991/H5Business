...
import com.park.scanpay.config.PHPConfig;
import com.park.scanpay.profile.URLProfile;
import com.park.scanpay.request.UpdateOrderRequest;
import com.park.scanpay.response.OrderResponse;
import com.park.scanpay.response.PHPResponse;
...
//		String key =parkDao.getParkKey(scanpayVO.getParkid());
		...
		String result =HttpTools.HttpClientGet(requestURL, params,map);
...
//				+ "}";
	...
	public String updateOrder(String order_num,String cost_after,int uid,float balance){
		...
		String result =HttpTools.HttpClientPost(requestURL,DateChangeTools.bean2gson(updateOrderRequest),getMap(requestURL));
		logger.info("***支付成功后更改订单状态返回结果：" + result + "，order_num：" + order_num);
		if (null ==result ||  "".equals(result)) {
			logger.error("*** 支付成功后更改订单状态返回结果为空：" + result + "，order_num：" + order_num);
			result="";
		}
		return result;	
	}
	...
	public String weixinnotify(String out_trade_no,String total_fee,String prepayid){
		...
		String result =HttpTools.HttpClientGet(requestURL, params,getMap(requestURL+"?"+params));
		logger.info("*** 修改订单返回结果：" + result + "，out_trade_no：" + out_trade_no);
		if (null ==result ||  "".equals(result)) {
			logger.error("*** 修改订单返回结果为空：" + result + "，out_trade_no：" + out_trade_no);
			result="";
		}
		return result;
	}
